...
> $$
> \begin{align} \text{Given }y>1, \text{ define }C:&&C&=\left\{ x\in  \mathbb{R}: x>0\text{ and }x^{2}<y \right\} \\ \text{Let }\alpha=\text{sup }C\text{, and define }\beta:&& \beta&= \alpha+\left(  \frac{y-\alpha^{2}}{\alpha+y}  \right) = \frac{y( \alpha+1 )}{\alpha+y} \\ \text{Then}:&&\beta^{2}-y&=\frac{y( y-1 )( \alpha^{2}-y )}{( \alpha+y )^{2}} \end{align}
> $$
> ... now observe:
> - If $\alpha^{2}<y$, then:
> 	- $\beta>\alpha$
