...
#### Preface
Let $( s_{n} )_{n\ge 1}$ be a sequence in $\mathbb{R}$. Then, for $k\ge 1$,
$$
\begin{align} a_{k}&=\text{inf}\left\{ s_{n}:n\ge k \right\} \\ b_{k}&=\text{sup}\left\{ s_{n}:n\ge k \right\} \end{align}.
$$
Suppose there's some sequence $E_{k}:$
$$E_{k}=\left\{ s_{n}:n\ge k \right\}=\left\{ s_{k}, s_{k+1}, s_{k+2}, \cdots \right\}$$
... where $E_{k+1}\subseteq E_{k}\longrightarrow a_{k}\le a_{k+1}$.
**Why?**
...
> - Take limits:
> 	$$
> 	\begin{align} \lim_{k \to \infty}( \alpha_{k}+\beta_{k} )&\le \varliminf_{n \to \infty}( a_{n}+b_{n} ) \\ &= \lim_{k \to \infty}\alpha_{k}+\lim_{k \to \infty}\beta_{k} \\ &= \varliminf_{n \to \infty}a_{n}+\varliminf b_{n} .\end{align}
> 	$$
> 	... **qed**
