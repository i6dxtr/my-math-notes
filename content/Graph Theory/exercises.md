...
~~~ad-example
title: Exercise 18. Let $G$ be an $X, Y$-bipartite graph and let $d = \max\{|S| - |N(S)| : S \subseteq X\}.$
...
~~~ad-example
title: Exercise 22. *(Expansion Lemma)*
...
~~~ad-example
title: Exercise 23. Let $G$ be a graph with at least four vertices. Prove that the following are equivalent:
...
~~~ad-example
title: Exercise 24. Let $k$ and $n$ be integers with $1 \le k \le n-1.$ If $G$ is a graph on $n$ vertices with $\delta(G) \ge \frac{n+k-2}{2},$ then $\kappa(G) \ge k.$
...
~~~ad-example
title: Exercise 25. Let $G$ be a $k$-connected graph.
...
~~~ad-example
title: Exercise 26. _For all graphs $G$ and all $xy \in E(G),$ $\kappa(G) \ge \kappa(G-xy) \ge \kappa(G) - 1.$_
