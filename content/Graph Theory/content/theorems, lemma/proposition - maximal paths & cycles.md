...
> Since $P$ is maximal every neighbor of $v_1$ and every neighbor of $v_\ell$ lies on $P$. Hence $S\cup T\subseteq\{v_1,\dots,v_{\ell-1}\}$. Compute
> $$
> \begin{align} |S\cap T|&=|S|+|T|-|S\cup T|\\ &\ge \delta(G)+\delta(G)-(\ell-1)\\ &\ge 2\delta(G)-(2\delta(G)-1)\\ &=1. \end{align}
> $$
...
### Relations
- Builds on maximal-path arguments in [[path|path]].
- Used to derive long-path and Hamiltonicity type consequences in conjunction with degree bounds ([[theorem - minimum degree & path-cycle length|minimum degree → long paths/cycles]]).
